...
export default function App() {
  ...
  async function performLookup(nextQuery, requestedType = selectedType, options = {}) {
    ...
    if (!trimmed) {
      setStatusText("Nejdřív zadej SPZ nebo VIN.");
      setError("");
      setResult(null);
      return;
    }
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  <div className="relative flex-1">
                    <Search className="pointer-events-none absolute left-5 top-1/2 size-5 -translate-y-1/2 text-muted-foreground" />
	                    <Input
	                      aria-label="Zadejte SPZ nebo VIN"
	                      autoComplete="off"
                      className={cn(
                      ...
                      placeholder="SPZ nebo VIN"
                      value={query}
                    />
                  </div>

                  <Button className={cn("rounded-full px-6 sm:px-7", hasSearched ? "h-12" : "h-14")} size="lg" type="submit">
                    {loading ? (
                      <>
...
function VehicleDimensionsPanel({ dimensions }) {
  const specs = [
    { label: "Délka", value: dimensions.length, icon: "length" },
    { label: "Šířka", value: dimensions.width, icon: "width" },
    { label: "Výška", value: dimensions.height, icon: "height" },
    { label: "Rozvor", value: dimensions.wheelbase, icon: "wheelbase" },
    { label: "Hmotnost", value: dimensions.weight, icon: "weight" }
    ...
    <Card className="scroll-mt-24 border-white/10 bg-card/80" id="dimensions">
      ...
      <CardContent>
        <div className="rounded-[1.5rem] bg-[#f8f8f6] p-3 text-[#0b0f17] shadow-soft sm:p-4">
          <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_300px]">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
              ...
              <div className="flex min-h-[180px] items-center justify-center rounded-[1.25rem] border border-slate-200 bg-white p-5">
                <DimensionIcon type="weight" className="mr-5 size-16 text-blue-800" />
                <div>
          ...
          <div className="mt-4 grid overflow-hidden rounded-[1.25rem] border border-slate-200 bg-white sm:grid-cols-2 lg:grid-cols-5">
            {specs.map((item, index) => (
              <div className={cn("flex items-center gap-4 px-5 py-4", index ? "border-t border-slate-200 sm:border-l sm:border-t-0" : "")} key={item.label}>
                <DimensionIcon type={item.icon} className="size-12 shrink-0 text-blue-800" />
                <div>
                  <p className="text-xl font-semibold text-blue-900">{item.label}</p>
...
function DimensionIcon({ type, className }) {
  ...
  if (type === "wheelbase") {
    return (
      <svg aria-hidden="true" className={className} viewBox="0 0 72 72">
        <rect fill="none" height="42" rx="7" stroke="currentColor" strokeWidth="7" transform="rotate(-42 36 36)" width="18" x="27" y="15" />
        <path d="M29 23h8M29 35h8M29 47h8" stroke="currentColor" strokeLinecap="round" strokeWidth="5" transform="rotate(-42 36 36)" />
      </svg>
    );
  }

  const vertical = type === "height";
  return (
    <svg aria-hidden="true" className={className} viewBox="0 0 72 72">
      <path d={vertical ? "M36 8v56M20 23 36 7l16 16M20 49l16 16 16-16" : "M8 36h56M23 20 7 36l16 16M49 20l16 16-16 16"} fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="7" />
    </svg>
  );
}
...
function IcoLookupLink({ ico, emptyText = "IČO není ve zdroji" }) {
  ...
  return <span className="break-all font-medium text-muted-foreground">IČO {normalizedIco}</span>;
}
